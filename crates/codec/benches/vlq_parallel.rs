...
use srcmap_codec::{Segment, SourceMapMappings, encode, encode_parallel};

/// Realistic mappings with varied deltas; `lines` rows of
/// `base_segments + line % segment_spread` segments each.
fn make_realistic_mappings(
    lines: i64,
    base_segments: usize,
    segment_spread: i64,
) -> SourceMapMappings {
    let mut mappings = Vec::with_capacity(lines as usize);
    let mut src: i64 = 0;
    ...
    for line_idx in 0..lines {
        let segments_per_line = base_segments + (line_idx % segment_spread) as usize;
...
fn make_large_realistic_mappings() -> SourceMapMappings {
    make_realistic_mappings(5000, 10, 40)
}

fn make_very_large_realistic_mappings() -> SourceMapMappings {
    make_realistic_mappings(50000, 5, 20)
}
